...
const sell = async ({ args }, rpc, ownPubKeyBuffer, sellingsBee) => {
  // const [ticker, price] = args;
  // const sellOrder = { ticker, price, owner: ownPubKeyBuffer.toString("hex") };
  // console.log("sell order", sellOrder);
  // const rawData = Buffer.from(JSON.stringify(sellOrder), "utf-8");
  // await sellingsBee.put(sellOrder.ticker, rawData);
  const [ticker, price] = args;
...
const bid = async ({ args }, rpc, _ownPubKeyBuffer) => {
  ...
  console.log("Made a bid!!", resp);
};

const terminate = async ({ args }, rpc, ownPubKeyBuffer) => {
  const [ticker] = args;
  console.log("terminate", { ticker });
};
...
const onCommand = async (
  ...
  switch (command) {
    case "ping":
      ...
      break;
    case "sell":
      await sell({ args }, rpc, ownPubKeyBuffer, sellingsBee);
      break;
    case "bid":
      await bid({ args }, rpc, ownPubKeyBuffer, sellingsBee);
      break;
    case "terminate":
      await terminate({ args }, rpc, ownPubKeyBuffer, sellingsBee);
      break;
    case "exit":
