...
const sell = async ({ args }, rpc, ownPubKeyBuffer) => {
  ...
  console.log("Created sell order", res);
};

const bid = async ({ args }, rpc, ownPubKeyBuffer) => {
...
const onCommand = async ({ command, args }, rpc, ownPubKeyBuffer) => {
  switch (command) {
    case "ping":
      ...
      break;
    case "sell":
      await sell({ args }, rpc, ownPubKeyBuffer);
      break;
    case "bid":
      await bid({ args }, rpc, ownPubKeyBuffer);
      break;
    case "terminate":
